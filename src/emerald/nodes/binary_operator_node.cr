class BinaryOperatorNode < Node
  ...
  def resolve_binary_llvm_values(state : ProgramState, lhs, rhs) : Nil
    case lhs.type
    when state.int32
      if rhs.type == state.int32
        resolve_binary_int32s state, lhs, rhs
      elsif rhs.type == state.double
