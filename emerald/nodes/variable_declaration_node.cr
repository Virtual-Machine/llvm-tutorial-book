class VariableDeclarationNode < Node
  ...
  def resolve_value(state : ProgramState) : Nil
    ...
    if state.variable_exists? state.active_function, @value.as(String)
      state.builder.position_at_end state.active_block
      state.builder.store (crystal_to_llvm state, @resolved_value), state.variable_pointers[state.active_function][@value.as(String)]
    end
