...
In our case, we are not going to simplify our AST prior to code generation. While this may result in slightly more work in our code generation it will not result in a larger binary. When using Crystal's builder bindings, simple mathematic operations are not appended to the active block; instead you capture the result and use it in subsequent instructions directly. We can likely still benefit from LLVM optimizations on the outputted IR regardless of approach. Feel free to experiment with simplifying your AST prior to code generation if you want to take the example compiler further.
